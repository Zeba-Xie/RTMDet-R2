...
@MODELS.register_module()
class RotatedIoULossTest(nn.Module):
    """RotatedIoULoss.
    ...
    def __init__(self,
                 ...
                 loss_weight=1.0,
                 mode='log'):
        super(RotatedIoULossTest, self).__init__()
        assert mode in ['linear', 'square', 'log']
        if linear:
            mode = 'linear'
            warnings.warn('DeprecationWarning: Setting "linear=True" in '
    ...
    def forward(self,
        ...
        if avg_factor is None:
            avg_factor = 1.0
        loss = self.loss_weight * (torch.sum(1 - rbbox_overlaps(pred, target, 'iou', True))[None] / avg_factor)
        return loss