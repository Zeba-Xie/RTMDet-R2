...
data = dict(
    ...
    test=dict(  # Test dataset config, modify the ann_file for test-dev/test submission
        ...
        version='oc'))
evaluation = dict(  # The config to build the evaluation hooks
...
optimizer_config = dict(  # Config used to build the optimizer hooks
    grad_clip=dict(
        max_norm=35,
        norm_type=2))
lr_config = dict(  # Learning rate scheduler config used to register LrUpdater hooks
...
checkpoint_config = dict(  # Config to set the checkpoint hooks
    interval=12)  # The save interval is 12
log_config = dict(  # config to register logger hooks
