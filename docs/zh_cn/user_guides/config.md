...
data = dict(
    ...
    test=dict(  # 测试数据集配置，修改测试开发/测试(test-dev/test)提交的 ann_file
        ...
        version='oc'))
evaluation = dict(  # evaluation hooks 的配置
    interval=12,  # 验证的间隔
...
optimizer_config = dict(  # optimizer hooks 的配置文件
    grad_clip=dict(
        max_norm=35,
        norm_type=2))
lr_config = dict(  # 学习率调整配置，用于注册 LrUpdater hooks
...
checkpoint_config = dict(  # checkpoint hooks 的配置文件
    interval=12)  # 保存的间隔是 12
log_config = dict(  # register logger hooks 的配置文件
    interval=50,  # 打印日志的间隔
    hooks=[
